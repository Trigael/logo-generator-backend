    ...
    createWholePrompt(body: GenerateLogoDto, amount: number, version?: number) {
        switch(version) {
            ...
            case 4: // Flux.1 [dev] 3 | last update: 11.08.25
                return `
                Details before generation: No other text than company ${body.slogan ? 'and slogan' : ''} (must be included) + ${body.additional_details} + ${body.things_to_exclude}
                ;Logo on a white background, transparent background, 
