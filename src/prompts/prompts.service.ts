...
export class PromptsService {
    ...
    createWholePrompt(body: GenerateLogoDto, amount: number, version?: number) {
        switch(version) {
            ...
            case 3: // ChatGPT -> Flux.1 [dev] 3 | last update: 08.08.25
                return `You are a highly experienced prompt engineer for FLUX.1[dev] 3.
                        Your task: Generate ${amount} concise, unique prompts for AI image generation of a flat, scalable logo for "${body.brand_name}".
                                
                        ---
                                
                        ### RULES FOR EVERY PROMPT:
                        - Start each prompt with: "Include only the text '${body.brand_name}' as the only text in the design. NO OTHER TEXT ANYWHERE."
                        - Exact brand name only — no ™, ©, ®, slogans, random letters, credits, captions, or extra symbols.
                        - Solid pure white (#FFFFFF) background only — no gradients, tints, patterns, or transparency.
                        - Single, centered, flat, isolated logo only.
                        - No mockups, frames, shadows, 3D, or realistic scenes.
                        - Avoid any micro text, hidden marks, or watermark-like patterns.
                        ...
                        ${body.additional_details ? `- Extra: ${body.additional_details}` : ''}
                        ${body.things_to_exclude ? `- Avoid: ${body.things_to_exclude}` : ''}
                                
                        ---
                                
                        ### NEGATIVE TOKENS:
                        Append at the end of each prompt:
                        --no extra text --no captions --no credits --no footers --no micro text --no watermarks --no placeholder text --no symbols
            ...
            case 5: // Flux.1 [dev] 3 (direct prompt) | last update: 11.08.25
                return `
                Rasterized, ${body.logo_style} vector company logo with its name '${body.brand_name}' and company slogan '${body.slogan}' using ${body.brand_colors} colors on transparent background. 
                -- Additional details about logo: ${body.additional_details}. 
                -- Negatives: text, symbols, ${body.things_to_exclude}.` 
            default:
